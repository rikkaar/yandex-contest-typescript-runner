...
### Make-формат (для задач с функциями)

Для некоторых задач (например, работа с структурами данных) требуется экспортировать только функцию `solution`, которая принимает аргументы напрямую:

Каждое решение оформляется как TypeScript-модуль, который экспортирует функцию `solution` с типом `SolutionFunction<Input, Output>`.

При сборке с флагом `-r make` создается правильная обёртка для платформы.

**Важно:** Некоторые задачи требуют вывод через `console.log` вместо возврата значения. В таких случаях используйте Jest spy для тестирования вывода.
...
| `-r, --runner <type>`      | Тип раннера (`file`, `std` или `make`)                                                   | `file`                    |
...
- **`file`** — используется для задач с чтением/записью из файлов (стандартный формат)
- **`std`** — используется для задач с чтением/записью из stdin/stdout (стандартный формат)
- **`make`** — используется для задач с функциями, где нужно экспортировать только `solution` (make-формат)
...
| `-t, --type <type>` | Тип шаблона (`default` или `make`) | `default` |

Команда автоматически создаёт директорию src/\<taskName> и копирует в неё все файлы из соответствующей директории шаблона:
- `_template` — для стандартных задач (с чтением/записью данных)
- `_template-make` — для задач с функциями (make-формат)
